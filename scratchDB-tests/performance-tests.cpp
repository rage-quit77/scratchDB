...
#include "scratchDB/scratchDBext.h"

TEST_CASE("Measure basic performance","[setKeyValue,getKeyValue]") {

  // Story:-
  //   [Who]   As a database administrator
  //   [What]  I need to know the performance characteristics of each scratchDB storage class
  ...
  SECTION("Store and Retrieve 100 000 keys - In-memory key-value store") {
    ...
    std::string dbname("myemptydb");
    std::unique_ptr<scratchDB::KeyValueStore> memoryStore = std::make_unique<scratchDBext::MemoryKeyValueStore>();
    std::unique_ptr<scratchDB::IDatabase> db(scratchDB::scratchDB::createEmptyDB(dbname, memoryStore));
  ...
  SECTION("Store and Retrieve 100 000 keys - File based key-value store") {
    ...
    std::string dbname("myemptydb");
    std::string fullpath = ".scratchDB/" + dbname;
    std::unique_ptr<scratchDB::KeyValueStore> memoryStore = std::make_unique<scratchDBext::FileKeyValueStore>(fullpath);
    std::unique_ptr<scratchDB::IDatabase> db(scratchDB::scratchDB::createEmptyDB(dbname, memoryStore));
